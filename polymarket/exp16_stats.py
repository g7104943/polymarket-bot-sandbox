#!/usr/bin/env python3
"""
Exp16 专用统计脚本 — 11 个组合的全方位分析（T+0 + TV宏观, 365天训练窗口）
...
用法：
  python3 polymarket/exp16_stats.py
  或 cd polymarket && python3 exp16_stats.py
"""
...
# Exp16 初始资金
EXP16_INITIAL_CAPITAL = 800

# 全部 11 个 Exp16 组合
EXP16_LOGS = [
    "logs_v5_exp16_bp0450",
    "logs_v5_exp16_bp0460",
    "logs_v5_exp16_bp0470",
    "logs_v5_exp16_bp0480",
    "logs_v5_exp16_bp0490",
    "logs_v5_exp16_bp0500",
    "logs_v5_exp16_bp0510",
    "logs_v5_exp16_bp0520",
    "logs_v5_exp16_bp0530",
    "logs_v5_exp16_bp_dyn_0450_0530",
    "logs_v5_exp16_bp_dyn_0480_0510",
]
...
def short_name(log_dir: str) -> str:
    return log_dir.replace("logs_v5_exp16_", "")
...
def load_report_summary(log_dir: str) -> dict | None:
    ...
    if current_capital is not None:
        ...
        fc = round(EXP16_INITIAL_CAPITAL + float(total_pnl), 2)
    else:
        return None
    pnl_val = float(total_pnl) if total_pnl is not None else (fc - EXP16_INITIAL_CAPITAL)
    return {
...
def main():
    print()
    print("=" * 74)
    print("  Exp16 专用统计（11 个组合 · 初始资金 $800 · T+0 + TV宏观）")
    print("=" * 74)

    # ━━━ 板块 1：按最终资金排名 ━━━
    combo_stats = []
    for log_dir in EXP16_LOGS:
        report = load_report_summary(log_dir)
        if report:
            xrp_pnl = _get_xrp_pnl(log_dir)
            report["total_pnl"] -= xrp_pnl
            report["final_capital"] = EXP16_INITIAL_CAPITAL + report["total_pnl"]
        ...
        combo_stats.append({
            ...
            "win_rate": wr,
            "total_pnl": total_pnl,
            "final_capital": round(EXP16_INITIAL_CAPITAL + total_pnl, 2),
