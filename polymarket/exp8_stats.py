...
用法：
  python3 polymarket/exp8_stats.py
  或 cd polymarket && python3 exp8_stats.py
"""
...
EXP8_LOGS = [
    ...
    "logs_v5_exp8_bp0520",
    "logs_v5_exp8_bp0530",
    "logs_v5_exp8_bp_dyn_0450_0530",
    "logs_v5_exp8_bp_dyn_0480_0510",
]
...
def short_name(log_dir: str) -> str:
    return log_dir.replace("logs_v5_exp8_", "")
...
def load_report_summary(log_dir: str) -> dict | None:
    ...
    if current_capital is not None:
        ...
        fc = round(EXP8_INITIAL_CAPITAL + float(total_pnl), 2)
    else:
        return None
    pnl_val = float(total_pnl) if total_pnl is not None else (fc - EXP8_INITIAL_CAPITAL)
    return {
...
def main():
    print()
    print("=" * 74)
    print("  Exp8 专用统计（11 个组合 · 初始资金 $800）")
    print("=" * 74)

    # ━━━ 板块 1：按最终资金排名 ━━━
    combo_stats = []
    for log_dir in EXP8_LOGS:
        report = load_report_summary(log_dir)
        if report:
            xrp_pnl = _get_xrp_pnl(log_dir)
            report["total_pnl"] -= xrp_pnl
            report["final_capital"] = EXP8_INITIAL_CAPITAL + report["total_pnl"]
        ...
        combo_stats.append({
            ...
            "win_rate": wr,
            "total_pnl": total_pnl,
            "final_capital": round(EXP8_INITIAL_CAPITAL + total_pnl, 2),
