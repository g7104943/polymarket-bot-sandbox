...
# Research-only integrated top159 main-model retraining.
...
THREADS = os.environ.get('TOP159_MAIN_WORKER_THREADS', '2')
...
WORKERS = int(os.environ.get('TOP159_MAIN_WORKERS', '4'))
MAX_SECONDS = int(os.environ.get('TOP159_MAIN_MAX_SECONDS', str(24 * 3600)))
CHECKPOINT_SECONDS = int(os.environ.get('TOP159_MAIN_CHECKPOINT_SECONDS', '900'))
MC_TRIALS = int(os.environ.get('TOP159_MAIN_MC_TRIALS', '800'))
MAX_TRAIN_ROWS = int(os.environ.get('TOP159_MAIN_MAX_TRAIN_ROWS', '180000'))
ENGINES = [x.strip() for x in os.environ.get('TOP159_MAIN_ENGINES', 'lightgbm,xgboost,logistic').split(',') if x.strip()]
PARAM_LIMIT = int(os.environ.get('TOP159_MAIN_PARAM_LIMIT', '0'))
RNG_SEED = 20260502

OUT_RESULTS = REPORTS / 'top159_integrated_main_model_results_latest.jsonl'
OUT_CHECKPOINT = REPORTS / 'top159_integrated_main_model_checkpoint_latest.json'
OUT_LEADERBOARD_JSON = REPORTS / 'top159_integrated_main_model_leaderboard_latest.json'
OUT_LEADERBOARD_MD = REPORTS / 'top159_integrated_main_model_leaderboard_latest.md'
OUT_AUDIT_JSON = REPORTS / 'top159_integrated_main_model_bug_audit_latest.json'
OUT_VERDICT_JSON = REPORTS / 'top159_integrated_main_model_unique_verdict_latest.json'
OUT_VERDICT_MD = REPORTS / 'top159_integrated_main_model_unique_verdict_latest.md'
...
def feature_subset(features: list[str], mode: str) -> list[str]:
    if mode == 'base15':
        ...
        out = list(features)
    # Cap feature count deterministically by variance. This prevents noisy wide modes from dominating.
    return out[: int(os.environ.get('TOP159_MAIN_MAX_FEATURES', '120'))]
...
def param_grid() -> list[dict[str, Any]]:
    ...
    for engine in engines:
        train_windows = ['1y', '2y', '3y', '5y', '7y', 'full']
        feature_modes = ['base15', 'base_plus_1h4h', 'trend_multi', 'daily_heavy', 'wide']
        edges = [0.02, 0.03, 0.04, 0.045, 0.05, 0.06, 0.075, 0.09, 0.10]
        for tw, fm, edge, vol_q in itertools.product(train_windows, feature_modes, edges, [0.80, 0.90, 0.95, 0.999]):
            if engine == 'logistic':
                rows.append({'engine': engine, 'train_window': tw, 'feature_mode': fm, 'edge': edge, 'vol_q': vol_q, 'n_estimators': 1, 'learning_rate': 0.04, 'num_leaves': 16, 'min_child_samples': 60, 'subsample': 0.9, 'colsample_bytree': 0.9, 'reg_lambda': 1.0, 'depth': 3, 'C': 1.0})
                continue
            for ne, lr, leaves, mcs, reg in [(100, 0.035, 24, 80, 0.5), (160, 0.025, 32, 60, 1.0), (220, 0.02, 40, 100, 2.0), (120, 0.055, 20, 120, 0.8)]:
                rows.append({'engine': engine, 'train_window': tw, 'feature_mode': fm, 'edge': edge, 'vol_q': vol_q, 'n_estimators': ne, 'learning_rate': lr, 'num_leaves': leaves, 'min_child_samples': mcs, 'subsample': 0.88, 'colsample_bytree': 0.88, 'reg_lambda': reg, 'depth': 4})
    if PARAM_LIMIT > 0:
...
def bug_audit(df: pd.DataFrame, features: list[str], base_by_window: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    repeat = []
    params = {'engine': 'logistic', 'train_window': '5y', 'feature_mode': 'base_plus_1h4h', 'edge': 0.045, 'vol_q': 0.999, 'n_estimators': 1, 'learning_rate': 0.04, 'num_leaves': 16, 'min_child_samples': 60, 'subsample': 0.9, 'colsample_bytree': 0.9, 'reg_lambda': 1.0, 'depth': 3, 'C': 1.0}
    ...
    audit['passed'] = not forbidden_hits and audit['repeatabilityPassed']
...
def run() -> int:
    started = time.time()
    print(f'[integrated-main] start {bj_now()} workers={WORKERS} maxSeconds={MAX_SECONDS}', flush=True)
    df, features, data_truth = build_integrated_frame()
    base_by_window = base_rows()
    audit = bug_audit(df, features, base_by_window)
    print(f'[integrated-main] features={len(features)} rows={len(df)} auditPassed={audit["passed"]}', flush=True)
    params = param_grid()
    total = len(params)
    ...
    for i, p in enumerate(params):
        ...
        if name not in done_names:
            pending.append((i, p))
    print(f'[integrated-main] total={total} done={len(results)} pending={len(pending)}', flush=True)
        ...
        with cf.ProcessPoolExecutor(max_workers=WORKERS, initializer=_init_worker, initargs=(df, features)) as ex:
            ...
            while futures:
                ...
                if time.time() - last >= CHECKPOINT_SECONDS:
                    write_progress(results, base_by_window, total, started, {**data_truth, 'bugAudit': audit}, finished=False)
                    print(f'[integrated-main] checkpoint {len(results)}/{total}', flush=True)
                    last = time.time()
            for fut in futures:
                fut.cancel()
