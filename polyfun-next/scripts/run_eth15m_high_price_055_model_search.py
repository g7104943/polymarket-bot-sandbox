...
"""ETH 15m high-price 0.55 main-model search.
...
User-requested fair metric:
  ...
  - primary buy price 0.55, auxiliary 0.50 / 0.60
  - full fill, ignore winner/loser fill toxicity
  - model chooses UP / DOWN / NO_TRADE

Important math:
  - At buy price 0.55, a win earns (1/0.55 - 1) = 81.818% of stake.
  - A loss loses 100% of stake.
  - Break-even win rate is exactly 55% before other costs.
...
THREADS = os.environ.get("ETH15M_BUY055_THREADS", "1")
...
WORKERS = int(os.environ.get("ETH15M_BUY055_WORKERS", "4"))
MAX_SECONDS = int(os.environ.get("ETH15M_BUY055_MAX_SECONDS", str(4 * 3600)))
PARAM_LIMIT = int(os.environ.get("ETH15M_BUY055_PARAM_LIMIT", "900"))
MAX_TRAIN_ROWS = int(os.environ.get("ETH15M_BUY055_MAX_TRAIN_ROWS", os.environ.get("ETH15M_ORDERFLOW_MAX_TRAIN_ROWS", "180000")))
RNG_SEED = 20260506
START_BANKROLL = 850.0
STAKE_PCT = 0.01
PRIMARY_BUY_PRICE = 0.55
BUY_PRICES = [0.55, 0.50, 0.60]
MAX_ALLOWED_DRAWDOWN_USD = START_BANKROLL * 0.25
TARGET_DRAWDOWN_USD = START_BANKROLL * 0.15

OUT_DATA_TRUTH_MD = REPORTS / "eth15m_buyprice055_data_truth_latest.md"
OUT_DATA_TRUTH_JSON = REPORTS / "eth15m_buyprice055_data_truth_latest.json"
OUT_LABEL_AUDIT_MD = REPORTS / "eth15m_buyprice055_label_audit_latest.md"
OUT_LABEL_AUDIT_JSON = REPORTS / "eth15m_buyprice055_label_audit_latest.json"
OUT_RESULTS = REPORTS / "eth15m_buyprice055_results_latest.jsonl"
OUT_LEADERBOARD_JSON = REPORTS / "eth15m_buyprice055_leaderboard_latest.json"
OUT_LEADERBOARD_MD = REPORTS / "eth15m_buyprice055_leaderboard_latest.md"
OUT_COMPARE_JSON = REPORTS / "eth15m_buyprice055_180_365_archive_compare_latest.json"
OUT_COMPARE_MD = REPORTS / "eth15m_buyprice055_180_365_archive_compare_latest.md"
OUT_VERDICT_JSON = REPORTS / "eth15m_buyprice055_unique_verdict_latest.json"
OUT_VERDICT_MD = REPORTS / "eth15m_buyprice055_unique_verdict_latest.md"
OUT_CHECKPOINT = REPORTS / "eth15m_buyprice055_checkpoint_latest.json"
...
def evaluate_params(df: pd.DataFrame, features: list[str], params: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if len(feats) < 8:
        return None
    name = (
        f"buyprice055_{params['engine']}_{params['train_window']}_{params['feature_mode']}"
...
def param_grid() -> list[dict[str, Any]]:
    engines = [x for x in os.environ.get("ETH15M_BUY055_ENGINES", "logistic,lightgbm,xgboost").split(",") if x]
    q_engines = [x for x in os.environ.get("ETH15M_BUY055_QUALITY_ENGINES", "logistic,lightgbm").split(",") if x]
    rows: list[dict[str, Any]] = []
    thresholds = [0.55, 0.56, 0.57, 0.58, 0.59, 0.60, 0.62, 0.64, 0.66, 0.68, 0.70]
...
def highprice_pass(candidate: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    for window, min_trades in [("180d", 100), ("365d", 200)]:
        ...
        if r["winRatePct"] <= 55.0:
            reasons.append(f"{window}_winrate_not_above_55")
        if r["compoundPnl"] <= 0:
    ...
    # 0.50 is easy sanity. 0.60 is reported as stress, not a hard gate.
    price_by = {(r["window"], r["buyPrice"]): r for r in candidate.get("priceRows", [])}
    for window in ["180d", "365d"]:
        if price_by.get((window, 0.50), {}).get("compoundPnl", -1e9) <= 0:
            reasons.append(f"{window}_buy050_not_positive")
...
def run_audit(df: pd.DataFrame, features: list[str]) -> dict[str, Any]:
    params = {
        ...
        "threshold": 0.55, "score_mode": "dir_only", "quality_weight": 0.0, "quality_threshold": 0.55,
    ...
    write_text(OUT_LABEL_AUDIT_MD, "\n".join([
        "# ETH15m 买价0.55模型：标签与回测审计", "",
        f"- 北京时间：`{audit['beijingTime']}`",
        f"- 主买价：`{PRIMARY_BUY_PRICE}`；打平胜率：`55%`；赢单收益/本金：`{audit['winReturnPerStake']}`",
...
def render_outputs(payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# ETH15m 买价0.55模型：180/365/归档对比", "",
        f"- 北京时间：`{payload['beijingTime']}`",
        "- 口径：`850U初始 / 每笔1% / 满成交 / 主买价0.55`",
        "- 数学：买价0.55时，赢单赚本金53.85%，输单亏100%，打平胜率55%。",
        "- live动作：`无，研究只读`", "",
        "## 最佳候选主表", "",
    ...
    if selected:
        for r in selected.get("priceRows", []):
            if r["buyPrice"] in BUY_PRICES:
                lines.append(f"|买价0.55最佳|{r['window']}|{r['buyPrice']:.2f}|{r['trades']}|{r['wins']}/{r['losses']}|{r['winRatePct']:.2f}%|{r['compoundPnl']:,.2f}|{r['endingBankroll']:,.2f}|{r['maxDrawdownUsd']:,.2f}|{r['returnDrawdownRatio']:.2f}|{r['monthlyPositiveRatio']:.2%}|`{r['setHash']}`|")
    ...
    lb_lines = ["# ETH15m 买价0.55模型：候选排行榜", "", "|排名|配置|训练窗|模型|特征组|阈值|校准|365胜率|365交易|365盈亏|365回撤|通过|", "|---:|---|---|---|---|---:|---|---:|---:|---:|---:|---|"]
    ...
    v_lines = [
        "# ETH15m 买价0.55模型：唯一结论", "",
        f"- 状态：`{verdict.get('status')}`",
        f"- 结论：{verdict.get('message')}", "",
        "## 说明", "",
        "- 这条线专门服务 `平均买价0.55`，不再用 `0.50` 高交易频率表硬凑。",
        "- 如果没有候选同时满足 180/365 胜率  >55%、正盈亏、低回撤、归档不差，就不能进入真钱。",
    ]
...
def write_progress(results: list[dict[str, Any]], total: int, start_ts: float, df: pd.DataFrame, features: list[str], data_truth: dict[str, Any], label_audit: dict[str, Any], finished: bool = False) -> None:
    ...
    status = "buyprice055_candidate_found" if live_eligible else "no_buyprice055_candidate"
    msg = "找到满足0.55高价口径的候选，只能进入24小时影子验证，不改真钱。" if live_eligible else "没有候选同时满足0.55买价下180/365胜率>55%、正盈亏、低回撤和归档复核。当前不应切真钱。"
    payload = {
        "generatedAt": now_iso(),
        "beijingTime": bj_now(),
        "researchOnlyNoLiveChange": True,
...
def main() -> None:
    ...
    data_truth.update({
        "generatedAtBuyPrice055": now_iso(),
        "researchOnlyNoLiveChange": True,
        "primaryBuyPrice": PRIMARY_BUY_PRICE,
        "auxBuyPrices": BUY_PRICES,
        "breakEvenWinRatePct": 55.0,
        "winReturnPerStakeAt055": round(1.0 / PRIMARY_BUY_PRICE - 1.0, 10),
        "note": "This script optimizes for high-price 0.55. It does not mutate current 061 live trading.",
    })
    write_json(OUT_DATA_TRUTH_JSON, data_truth)
    write_text(OUT_DATA_TRUTH_MD, "\n".join([
        "# ETH15m 买价0.55模型：数据真相", "",
        f"- 北京时间：`{bj_now()}`",
        f"- 最终样本：`{data_truth['finalRows']}`",
        f"- 特征数：`{data_truth['featureCount']}`",
        f"- 主买价：`{PRIMARY_BUY_PRICE}`；打平胜率：`55%`；赢单收益/本金：`{data_truth['winReturnPerStakeAt055']}`",
