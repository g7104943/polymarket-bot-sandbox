...
RAW_LOG_DIRS = {
    'BTC': PROJECT_ROOT / 'polymarket' / 'logs_vnext_btc_sequence_v3_raw',
    'ETH': PROJECT_ROOT / 'polymarket' / 'logs_vnext_eth_sequence_v3_raw',
}
OVERLAY_LOG_DIRS = {
    'BTC': PROJECT_ROOT / 'polymarket' / 'logs_vnext_btc_sequence_v3',
    'ETH': PROJECT_ROOT / 'polymarket' / 'logs_vnext_eth_sequence_v3',
}
PATH_FILES = {
    'BTC': PROJECT_ROOT / 'data' / 'polymarket_1m_btc_usdt.parquet',
    'ETH': PROJECT_ROOT / 'data' / 'polymarket_1m_eth_usdt.parquet',
}
LEDGER_PATH = PROJECT_ROOT / 'polymarket' / 'predictions_vnext_btceth_sequence_v3_ledger.json'
...
def _execution_adjusted_pnl(base_pnl: float, asset: str, exit_family: str, sell_quote_policy: str, spread_metric: float | None) -> tuple[float, dict[str, Any]]:
    ...
    adjusted = -0.02 if fill_rate <= 0.0 else float(base_pnl) * fill_rate * partial_ratio
...
def _overlay_entry(raw: dict[str, Any], ledger: dict[str, Any] | None, paths: dict[int, tuple[list[int], list[float]]]) -> dict[str, Any]:
    ...
    base['model_variant'] = str((ledger or {}).get('model_variant') or 'vnext_sequence_v3')
...
def main() -> int:
    ap = argparse.ArgumentParser(description='Reconcile sequence v3 overlay logs for monitor_all_orders')
    ap.add_argument('--once', action='store_true')
    args = ap.parse_args()
    if args.once:
        run_once()
        return 0
    err_path = PROJECT_ROOT / 'logs' / 'reconcile_vnext_btceth_sequence_v3_overlay.error.log'
