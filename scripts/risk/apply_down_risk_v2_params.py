...
DEFAULT_REPORT = PROJECT_ROOT / "reports" / "down_risk_v2_tuning_report.json"
...
def _apply_to_config(
    ...
    for name in active_names:
        ...
        cfg["downRiskEngine"] = "v2"
        cfg["downRiskStatsMode"] = "route"
        cfg["downRiskCheckSeconds"] = int(cfg.get("downRiskCheckSeconds") or 60)
        resolved_modes: List[str] = []

        mode_map = cfg.get("downRiskModeBySymbol")
        ...
        cfg["downRiskModeBySymbol"] = mode_map
        if resolved_modes:
            cfg["downRiskMode"] = resolved_modes[0] if len(set(resolved_modes)) == 1 else "shadow"

        d = cfg.get("downRiskBySymbol")
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Apply DOWN risk V2.2 tuning params to active trader configs")
...
def main() -> int:
    ...
    decision_targets = _targets_from_decision(args.decision_json, "down", profiles) if args.decision_json else None
    recent_targets = _targets_from_recent(args.recent_gain_json, "down", profiles) if args.recent_gain_json else None
    ...
    print("[OK] applied down risk v2 params")
    print(
