...
def _compute_up_pnl_proxy(d: pd.DataFrame) -> pd.Series:
    """
    UP 代理收益口径（与风控阈值同量纲）：
    1) 二元市场收益建模（单位=每 1 USDC 名义下注）
       - win: (1/up_conf) - 1
       - lose: -1
    2) 波动幅度因子：abs(log_return)/median_abs_return，截断到 [0.5, 3.0]
    """
    up_conf = d["up_conf"].clip(lower=0.05, upper=0.95)
    pnl_base = np.where(
        d["up_win"].astype(int).values == 1,
        (1.0 / up_conf.values) - 1.0,
        -1.0,
    )
...
def _build_rows_for_cfg(
    ...
    up_bound = _resolve_up_threshold(cfg)
    ...
    for symbol in allowed:
        ...
        d["trigger_up"] = d["pred_up"] & (d["up_conf"] >= up_bound)
        ...
        for r in d[[
            ...
            "up_conf", "up_conf_threshold", "up_win", "up_pnl_proxy",
        ]].to_dict("records"):
            ...
            rows.append(
                {
                    ...
                    "confidence": float(r["up_conf"]),
                    "up_conf_threshold": float(r["up_conf_threshold"]),
                    "win": int(r["up_win"]),
                    "pnl": float(r["up_pnl_proxy"]),
                }
            )
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Build UP risk replay panel from td365d prediction files")
    ap.add_argument("--output", type=Path, default=REPORTS_DIR / "up_risk_v2_panel.parquet")
    ap.add_argument("--meta-output", type=Path, default=REPORTS_DIR / "up_risk_v2_panel_meta.json")
...
def main() -> int:
    ...
    summary: Dict[str, Any] = {
        ...
        "assets": ["BTC", "ETH"],
        "pnlProxyFormula": {
            "base": "win=(1/up_conf)-1, lose=-1 (per 1 USDC nominal)",
            "magnitude": "abs(log_return)/median_abs_return clipped to [0.5,3.0]",
        },
        "upThresholdMapping": "probThresholdUp if set else probThreshold/rules min_conf",
        "profiles": {
