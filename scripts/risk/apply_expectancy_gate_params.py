...
DEFAULT_REPORT = PROJECT_ROOT / "reports" / "expectancy_gate_tuning_report.json"
...
def _targets_from_recent(path: Path, profiles: Set[str]) -> Dict[str, Set[str]]:
    ...
    for item in items:
        ...
        if not isinstance(layers, dict):
            continue
        layer_payload = layers.get("expectancy")
...
def _extract_cluster_params(
    report: Dict[str, Any],
    profiles: Set[str],
) -> Tuple[Dict[str, Dict[str, Any]], bool, Dict[str, List[str]]]:
    ...
    for profile in ("default", "70"):
        ...
        for symbol in TARGET_SYMBOLS:
            for direction in TARGET_DIRECTIONS:
                ...
                if vlist:
                    has_violation = True
    return params_by_cluster, has_violation, violations
...
def _apply_to_config(
    ...
    for name in active_names:
        ...
        cfg["expectancyGateStatsMode"] = "route"
        cfg["expectancyGateCheckSeconds"] = int(cfg.get("expectancyGateCheckSeconds") or 60)

        mode_map = cfg.get("expectancyGateModeBySymbolDirection")
        if not isinstance(mode_map, dict):
            mode_map = {}
        param_map = cfg.get("expectancyGateBySymbolDirection")
        ...
        for symbol in allowed_symbols:
            for direction in TARGET_DIRECTIONS:
                ...
                if target_symbols and symbol not in target_symbols:
                    mode_map.pop(sd_key, None)
                    param_map.pop(sd_key, None)
                    continue
                ...
                if current_params:
                    ...
                    param_map[sd_key] = merged
                    cells += 1

        cfg["expectancyGateModeBySymbolDirection"] = mode_map
        cfg["expectancyGateBySymbolDirection"] = param_map
        if resolved_modes:
            cfg["expectancyGateMode"] = resolved_modes[0] if len(set(resolved_modes)) == 1 else "shadow"
        touched += 1
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Apply expectancy gate params to active trader configs")
    ap.add_argument("--report", type=Path, default=DEFAULT_REPORT)
    # 默认走 auto，避免在存在违例时误切到 enforce
...
def main() -> int:
    ...
    params_by_cluster, has_violation, violations = _extract_cluster_params(report, profiles)
    ...
    print("[OK] applied expectancy gate params")
